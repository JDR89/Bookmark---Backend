...
export class ProductsService {
    ...
    try {
      ...
      if (isUUID(term)) {
        ...
        product = await queryBuilder
          .where('UPPER(title) = :term', { term: term.toUpperCase() })
          .orWhere('UPPER(slug) = :term', { term: term.toUpperCase() })
  ...
  private handleExceptions(error: any) {
    if (error.code === '23505') {
      throw new BadRequestException('Ya existe un producto con el mismo slug')
    }
